...
class MambaBlock(nn.Module):
    ...
    def __call__(self, x, hiddens=None):
        ...
        y, h = self.ssm(x, hiddens=hiddens)
    ...
    def ssm(self, x, hiddens=None):
        ...
        y, h = self.selective_scan(x, delta, A, B, C, D, hiddens=hiddens)  # This is similar to run_SSM(A, B, C, u) in The Annotated S4 [2]
        
        return y, h


    def selective_scan(self, u, delta, A, B, C, D, hiddens=None):
        ...
        print("self.hidden_state_method is", self.hidden_state_method)
        if self.hidden_state_method == "zero":
            ...
            h = np.stack(hs, axis=0)  # shape (b, l, d_in, n) # TODO, why axis=0
            y = np.einsum('b l d n, b l n -> b l d', h, C) + u * D

            return y, hiddens
            # return y, None
        elif self.hidden_state_method == "previous":
            deltaA = np.exp(np.einsum('b l d, d n -> b l d n', delta, A))
            deltaB_u = np.einsum('b l d, b l n, b l d -> b l d n', delta, B, u)

            # Need to add identity to deltaA's first timestep and previous hidden state to deltaB_u's first timestep
            zero_shape = deltaA.shape[0:1] + (1,) + deltaA.shape[2:]
            # print("zero_shape is", zero_shape)
            assert zero_shape == hiddens.shape, "zero_shape is {}, hiddens.shape is {}".format(zero_shape, hiddens.shape)
            deltaA = np.concatenate([np.zeros(zero_shape), deltaA], 
                axis = 1,
                ) # b * l + 1 * d * n
            deltaB_u = np.concatenate([hiddens, deltaB_u], axis=1)
            assert deltaA.shape == deltaB_u.shape, "deltaA.shape is {}, deltaB_u.shape is {}".format(deltaA.shape, deltaB_u.shape)

            _, hs = jax.lax.associative_scan(binary_operator, (deltaA, deltaB_u), axis=1)

            h = np.stack(hs, axis=0)[:,1:,:,:] # shape (b, l, d_in, n) # TODO, why axis=0
            y = np.einsum('b l d n, b l n -> b l d', h, C) + u * D # 
